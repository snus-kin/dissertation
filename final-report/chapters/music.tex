...
picture]{\node(1AL){\includegraphics[width=.3\textwidth]{audiopoc2}};}}%
\hspace*{2cm}%
\subfloat{\tikz[remember
picture]{\node(1AR){\includegraphics[width=.3\textwidth]{audiopoc1}};}}
...
Overall, the composition should enhance what is on the screen, using the
parameters that are being manipulated by the user to inform what sounds are
created for example: if the number of vertices in the shapes on the screen
changes the music should change accordingly.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{adsr}
...
be given to the phase angle that each are set to. Setting them to \verb|(i / NUM_OSCS) * 360| 
allows for there to be no harsh constructive or destructive interference between
the oscillators.
